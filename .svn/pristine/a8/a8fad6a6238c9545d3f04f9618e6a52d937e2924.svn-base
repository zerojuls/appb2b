...
class CI_DB_active_record extends CI_DB_driver {
	...
	/**
        ...
        * Description  :   Genera una arreglo para manipular store procedure
        * 
        * Company      :   Adexus Per̼ S.A.
        */
	public function query_sp($s_store_name, $ar_param)
	{
            $ar_result = array();
            $s_param = '';
            
            //genrando parametros
            for ($i = 1; $i <= count($ar_param); $i ++){
                if ($i == 1){
                    $s_param = '?';
                } else{
                    $s_param = $s_param.', ?';
            ...
            //si es mysqli
            $s_store_name = 'CALL '. $s_store_name . ' ( '.$s_param.' )';

            $query = $this->query($s_store_name, $ar_param);
        ...
        public function dropdown($sQuery, $sData = null) {
            ...
            if ( ! is_null($sData)){
                ...
                $sQuery = 'CALL '. $sQuery . ' ( '.$s_param.' )';
                
                $query = $this->query($sQuery, $sData);
            } else {
