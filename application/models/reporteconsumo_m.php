...
class Reporteconsumo_m extends CI_Model {
    ...
    public function get_reporteconsumo_list($arrUsuario, $arrFechaIni, $arrFechaFin){
        $ar_result = array();
        $sql = "SELECT  	CP.id_producto AS id_producto, PR.codigo AS codigo, PR.nombre AS producto, E.nombre AS envase, U.nombre AS unidad, SUM(CPD.cantidad) AS cantidad, CP.precio AS precio_venta, (SUM(CPD.cantidad) * CP.precio) AS total
                ...
                INNER JOIN      sucursal S ON CPG.id_cliente = S.id_cliente
    ...
    public function table_reporte_consumo($id_pedido){
        $ar_result = array();
        $sql = "SELECT      P.id_pedido AS id_pedido, 
                            P.fecha_registro AS fecha, LPAD(P.id_pedido,10,'0') AS codigo_pedido,
                            C.nombre AS centro_costo, SUM(PD.cantidad) AS cantidad 
                FROM        pedido P 
                INNER JOIN pedido_detalle PD ON P.id_pedido = PD.id_pedido
                INNER JOIN unidad_venta UV ON PD.id_tipo_precio = UV.id_tipo_precio AND PD.id_producto=UV.id_producto AND PD.id_envase=UV.id_envase AND PD.id_unidad=UV.id_unidad
                INNER JOIN  cliente C ON P.id_cliente = C.id_cliente
                INNER JOIN  producto PR ON PD.id_producto = PR.id_producto
                WHERE       P.estado NOT IN ('T') AND PD.id_producto = ".$id_pedido."
